...
Installable plugins that bundle the New Relic NRQL capability of
`@kavinga/commerce-tools` for [Claude Code](https://docs.anthropic.com/en/docs/claude-code/plugins)
...
| `.mcp.json` (Claude) / `mcp.json` (Cursor) | MCP server | Wires up the `newrelic` MCP server (`execute_nrql`, `get_account_id_by_project_id`). |
...
The MCP server is launched via `npx` from the published `@kavinga/commerce-tools`
package, so installing a plugin needs no local build.
...
`NEW_RELIC_ACCOUNT_ID`) in your environment; the MCP server config reads them
